...
class AssetsContentObject extends AbstractContentObject
{
    ...
    protected function include($path, $standalone): void
    {
        ...
        $standalone = empty($standalone) ? false : (bool) $standalone;
        ...
        foreach ($files as $file) {
            if (strlen($file) > 3) {    //  at less, file have 4 char lenth
                ...
                if (preg_match("/(jquery-)/i", $file)) { //jquery-|bootstrap|others  // file at first
                    $file_atStart[] = $file;
                } elseif (str_contains(strtolower($file), 'custom')) {    //file have 'custom' on name
                    $file_custom[] = $file;
                } elseif ($pos_mapping = str_contains(strtolower($file), 'mapping')) {    //file have 'mapping' on name
                    $file_mapping[] = $file;
                } else {
                    $file_top[] = $file;
                }
        ...
        if (substr((string) $extPath, -1) !== DIRECTORY_SEPARATOR) {
            $extPath .= DIRECTORY_SEPARATOR;    // if directory path haven't '/' at last we add it
        }
        ...
        } elseif (!str_contains(BackendUtility::getHostname(), '.e5j.de')) {
        ...
        foreach ($returnArray as $file) {
            if (strtolower(substr($file, -4)) === '.css') {    // for css files
                $pageRenderer->addCssFile($extPath . $file, 'stylesheet', 'all', '', true, false, '', false, '|');        // render css files
            }

            if (strtolower(substr($file, -3)) === '.js') {    // for js files
                $pageRenderer->addJsFooterFile($extPath . $file, 'text/javascript', true, false, '', false, '|', false, '');    // render js files
            }
        }
    }   
}
